...
exports.listen = function (server, mongoServer) {
	...
	io.sockets.on('connection', function (socket) {
		...
		socket.on('initRoomInfo', function (data, fn) {
			...
			//用户断开后，清楚逻辑
			handleClientDisconnection(socket, nickNames, onlineUserMap);
...
function assignGuestName(socket, nickNames) {
	...
	var onlineUserMap_length = Object.keys(onlineUserMap);
	console.log("onlineUserMap_length:"+onlineUserMap_length.length);
};
...
function handleClientDisconnection(socket) {
	socket.on('disconnect', function () {
		...
		delete onlineUserMap[nickNames[socket.id]];
